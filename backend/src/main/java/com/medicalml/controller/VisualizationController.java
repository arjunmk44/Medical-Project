...
public class VisualizationController {
    ...
    public ResponseEntity<?> getClusterMap() {
        ...
        return ResponseEntity.ok(ApiResponse.Success.builder()
                .data(Map.of(
                        "clusters", new Object[] {
                                Map.of("x", 1.2, "y", 0.8, "cluster", 0, "label", "Healthy", "size", 45),
                                Map.of("x", -0.5, "y", 1.5, "cluster", 1, "label", "Monitor Closely", "size", 30),
                                Map.of("x", 2.1, "y", -1.2, "cluster", 2, "label", "At Risk", "size", 20),
                                Map.of("x", -1.8, "y", -0.9, "cluster", 3, "label", "High Risk", "size", 5)
                        }))
    ...
    public ResponseEntity<?> getCorrelationHeatmap() {
        String[] biomarkers = { "SystolicBP", "DiastolicBP", "Glucose", "HbA1c", "LDL", "HDL", "BMI", "Creatinine" };
        double[][] matrix = {
    ...
    public ResponseEntity<?> getPopulationStats() {
        return ResponseEntity.ok(ApiResponse.Success.builder()
                .data(Map.of(
                        ...
                        "avgRiskScore", 0.42,
                        "activeAlerts", 8,
                        "riskDistribution", Map.of("Healthy", 65, "Monitor Closely", 30, "At Risk", 25)))
    ...
    public ResponseEntity<?> getSurvivalAnalysis() {
        ...
        try {
            var response = webClientBuilder.build()
                    ...
                    .block();
            // Return survival data structure
